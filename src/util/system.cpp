...
const char * const BITCOIN_CONF_FILENAME = "monkycoin.conf";
...
#else
    const char* pszModule = "monkycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "monkycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/monkycoin";
#else
    // Unix
    return pathRet / ".monkycoin";
#endif
#endif
}
