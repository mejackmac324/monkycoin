...
const QString BITCOIN_IPC_PREFIX("monkycoin:");
#ifdef ENABLE_BIP70
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/monkycoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/monkycoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/monkycoin-paymentrequest";
#endif
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start monkycoin: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("monkycoin://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'monkycoin://' is not a valid URI. Use 'monkycoin:' instead."),
    ...
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid monkycoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
void PaymentServer::LoadRootCAs(X509_STORE* _store)
{
    ...
    if (certFile != "-system-") {
        ...
        QSslSocket::setDefauMNCaCertificates(certList);
    } else
...
void PaymentServer::fetchPaymentACK(WalletModel* walletModel, const SendCoinsRecipient& recipient, QByteArray transaction)
{
    ...
    if (walletModel->wallet().getKeyFromPool(false /* internal */, newKey)) {
        ...
        const OutputType change_type = walletModel->wallet().getDefauMNChangeType() != OutputType::CHANGE_AUTO ? walletModel->wallet().getDefauMNChangeType() : walletModel->wallet().getDefaultAddressType();
